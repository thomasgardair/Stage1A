...
Nous allons ensuite appliquer la CKM et l'arrondi aleatoire à nos tableaux afin de pouvoir comparer les deux méthodes perturbatrices. Nous choisissons un arrondi aléatoire en base 10 et un variance de 6.25 et déviation de 10 pour la CKM. Par exemple notre premier tableau ,contenant données originales et perturbées, est présenté ici : 
...
Nous utilisons la mesure d'association, V de Cramer :

\[ 
CV = \sqrt{\frac{\chi^2}{n \cdot \text{min}(R-1, C-1)}}
\]

et définissons la perte d'information par la différence relative en pourcentage entre le tableau original et le tableau perturbé :
...
D'une part, la CKM indiquent que les tableaux ayant un petit nombre de cellules subissent une légère perte association contrairement à l'arrondi aléatoire qui subie une légère hausse d'association.
...
```{r, echo=FALSE}
DT ::datatable(all_statistiques %>% select(Tableau,Taille,Nb_obs_inf_10,Nb_obs_inf_10_ckm,Nb_obs_inf_10_alea,p_value_Wilcoxon_ckm, p_value_Wilcoxon_alea))%>%formatRound(columns = c('p_value_Wilcoxon_ckm', 'p_value_Wilcoxon_alea'), digits = 3)
```
